...
目的：一次性回答「EMA 交叉這條路是死是活」。

對每組 (fast, slow, trend) 參數跑完整 walk-forward，收集樣本外（quasi-OOS）
績效，最後排名。判讀原則：
...
def run_grid_search(
    ...
    *,
    ...
    """對 (fast, slow, trend) 笛卡兒積跑 walk-forward，回傳結果清單。

    只保留 fast < slow 的組合。trend=0 表示無濾網。
    """
    ...
    for fast in fast_periods:
        for slow in slow_periods:
            ...
            for trend in trend_periods:
                def factory(f=fast, s=slow, t=trend, tf=timeframe):
                    return EmaCrossover(fast_period=f, slow_period=s,
                ...
                results.append(GridResult(
                    fast=fast, slow=slow, trend=trend, timeframe=timeframe,
