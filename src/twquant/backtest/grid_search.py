"""EMA / KD+EMA 參數普查（grid search over walk-forward）。

目的：一次性回答「策略這條路是死是活」，以及 ATR 移動停利是否改善績效。

對每組參數跑完整 walk-forward，收集樣本外（quasi-OOS）績效，最後排名。
判讀原則：
- 若**所有**組合 OOS Sharpe 皆為負 → 此方向沒有 edge，應埋葬
...
from twquant.strategies.kd_ema import KdEmaCrossover
...
def run_kd_grid_search(
    bars: pd.DataFrame,
    timeframe: str,
    *,
    fast_periods: list[int],
    slow_periods: list[int],
    rsv_periods: list[int] | None = None,
    atr_periods: list[int] | None = None,
    atr_mults: list[float] | None = None,
    k_smooth: int = 3,
    d_smooth: int = 3,
    ...
    """對 (fast, slow, rsv_period, atr_period, atr_mult) 笛卡兒積跑 KdEmaCrossover
    walk-forward，回傳結果清單（依 OOS Sharpe 遞減排序）。

    進場條件：EMA fast/slow 黃金交叉 + KD(rsv_period, k_smooth, d_smooth) 黃金交叉同步。
    出場：ATR 停利（atr_period > 0）或 EMA 死亡交叉。
    """
    rsv_periods = rsv_periods or [9]
    atr_periods = atr_periods or [0]
    atr_mults   = atr_mults   or [2.0]

    results: list[KdGridResult] = []

    for fast in fast_periods:
        for slow in slow_periods:
            if fast >= slow:
                continue
            for rsv in rsv_periods:
                for atr_p in atr_periods:
                    mults = [0.0] if atr_p == 0 else atr_mults
                    for atr_m in mults:
                        def factory(
                            f=fast, s=slow, r=rsv,
                            ap=atr_p, am=atr_m,
                            ks=k_smooth, ds=d_smooth, tf=timeframe,
                        ):
                            return KdEmaCrossover(
                                fast_period=f, slow_period=s,
                                timeframe=tf, rsv_period=r,
                                k_smooth=ks, d_smooth=ds,
                                atr_period=ap, atr_mult=am,
                            )
                        ...
                        m = wf.combined_metrics
                        results.append(KdGridResult(
                            fast=fast, slow=slow, rsv_period=rsv,
                            atr_period=atr_p, atr_mult=atr_m,
                            timeframe=timeframe,
                            oos_total_return_pct=m.total_return_pct,
                            oos_sharpe=m.sharpe,
...
def kd_results_to_df(results: list[KdGridResult]) -> pd.DataFrame:
    rows = []
    for r in results:
        rows.append({
            "fast":       r.fast,
...
def verdict(results: list[GridResult] | list[KdGridResult], label: str = "策略") -> str:
    ...
    if len(positive) == 0:
        lines.append(f"判決：全軍覆沒 → {label} 在此時間框沒有 edge，應埋葬此方向。")
    elif pct_pos < 25:
