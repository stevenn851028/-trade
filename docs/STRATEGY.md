# 策略規格：EMA10/60 Crossover (15-Minute)

## 概要

一條短期 EMA（EMA10）與一條中長期 EMA（EMA60）的交叉策略，應用於台指期貨 15 分鐘 K 棒。設計哲學：**順勢、中速節奏、兼顧訊號密度與穩定度**。

## 基本資訊

| 項目 | 值 |
| --- | --- |
| 策略代號 | `ema_cross_15m_v1` |
| 標的 | 台指期貨（TXF 大台 / MXF 小台） |
| K 棒週期 | 15 分鐘 |
| 快線 | EMA10（2.5 小時記憶） |
| 慢線 | EMA60（15 小時記憶，約 0.8 個交易日） |
| 允許方向 | 僅做多（Phase 1） |
| 最大同時部位 | 1 口 |
...
- 歷史資料需要 **至少暖機 60 根**（約 1 個交易日）才產出第一個有效訊號
...
- **勝率**：35–45%（15 分鐘雜訊較多，勝率略低於 30 分鐘版本）
- **損益比**：1.5 – 2.5（贏的時候多）
- **月均交易次數**：15–30 次（訊號密度約為 30 分鐘版的 2 倍）
- **最大回撤**：15–30%（需以風控降低）
- **年化報酬**：目標 10–20%（扣除成本後）
